...
export default function AdminSeekersPage() {
  ...
  useEffect(() => {
    getAdminSeekers().then(setSeekers);
  }, []);
  ...
  const toggleBlockStatus = (seekerId: string) => {
    setSeekers((currentSeekers) =>
      currentSeekers.map((seeker) =>
        seeker.id === seekerId
          ? {
              ...seeker,
              status: seeker.status === "blocked" ? "active" : "blocked",
            }
          : seeker,
      ),
    );
    ...
    setSelectedSeeker((currentSeeker) =>
      currentSeeker?.id === seekerId
        ? {
            ...currentSeeker,
            status: currentSeeker.status === "blocked" ? "active" : "blocked",
          }
        : currentSeeker,
    );
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 p-5">
          <div>
            ...
            <p className="m-0 mt-1 text-size13 text-slate-500">
              عرض الحسابات، مراجعة تفاصيل البروفايل، وإدارة حالة الحظر.
            </p>
          </div>
                ...
                <TableRow key={seeker.id}>
                  ...
                  <TableCell className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => toggleBlockStatus(seeker.id)}
                        className={cn(
        ...
        {selectedSeeker ? (
          <DialogContent className="w-[min(820px,92vw)] min-w-0">
            <DialogHeader className="text-start">
              ...
              <DialogDescription>
                كل البيانات المسجلة في بروفايل الباحث داخل المنصة.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex flex-wrap justify-end gap-3">
              <button
                type="button"
                onClick={() => toggleBlockStatus(selectedSeeker.id)}
                className={cn(
