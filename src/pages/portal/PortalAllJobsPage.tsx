...
export default function PortalAllJobsPage({ page }: PortalAllJobsPageProps) {
  ...
  const jobQuery = usePortalJob(selectedJobId);
    ...
    if (jobQuery.isLoading) {
      return (
        <PortalJobsLoadingState
          ...
          description="جار تحميل تفاصيل الوظيفة المختارة..."
        />
      );
    }

    if (jobQuery.isError || !selectedJob) {
      return (
  ...
  if (jobsQuery.isLoading) {
    return (
      <PortalJobsLoadingState
        title={page.title}
        description="جار تحميل الوظائف المتاحة حالياً..."
      />
    );
  }
