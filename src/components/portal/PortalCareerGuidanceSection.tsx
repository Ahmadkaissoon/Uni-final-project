...
const initialThreadTitles = ["توقعات الأسعار خلال السنتين القادمة ...."];
        ...
        currentThreads.map((thread) =>
          thread.id === targetThreadId
            ? {
                ...thread,
                title:
                  thread.title === "محادثة جديدة" ? trimmedDraft : thread.title,
                ...
                {threads.map((thread) => {
                  ...
                  return (
                    <div
                      ...
                      className={
                        isSelected
                          ? "group flex w-full items-center gap-2 rounded-lg bg-warning-color px-3 py-2 text-white transition duration-200"
                          : "group flex w-full items-center gap-2 rounded-lg bg-transparent px-3 py-2 text-white/95 transition duration-200 hover:bg-white/8"
                      }
                    >
                      <button
                        type="button"
                        onClick={() => setSelectedThreadId(thread.id)}
                        className="min-w-0 flex-1 bg-transparent p-0 text-right text-xs font-medium text-inherit"
                      >
                      ...
                      <button
                        ...
                        className="pointer-events-none inline-flex size-7 shrink-0 items-center justify-center rounded-full text-white/80 opacity-0 transition duration-200 hover:bg-white/15 hover:text-white group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100"
...
function GuidanceComposer({
  ...
  return (
    <div
      className={
        centered ? "relative mx-auto w-full max-w-[290px]" : "relative w-full"
      }
    >
      <input
        type="text"
